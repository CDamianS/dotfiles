...
#+begin_src conf
set $term alacritty
set $eshell emacsclient -c -a 'emacs' --eval '(eshell)'
...
bindsym $mod+Shift+alt+w exec firefox-devedition
bindsym $mod+Shift+f exec $files
bindsym $mod+Shift+r exec $rss
bindsym $mod+Shift+a exec $agenda
...
** Menus
#+begin_src conf
bindsym $mod+d exec bemenu-run -l 10 -p 
...
bindsym XF86Display exec mpv ~/Movies/Scream\ \(1996\)/Scream\ \(1996\).mp4
